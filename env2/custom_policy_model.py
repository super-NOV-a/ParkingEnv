...
class RadarConvFusion(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=128):
        ...
        self.radar_conv = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=4, kernel_size=5, padding=2, padding_mode="circular"),
            nn.ReLU(),
            nn.Conv1d(4, 4, kernel_size=3, padding=1, padding_mode="circular"),
            nn.ReLU(),
            nn.Flatten(),  # 输出 shape = [batch, 8×72]
        )

        self.linear = nn.Sequential(
            nn.Linear(4 * 72 + 5, 256),
            nn.ReLU(),
