    ...
    while True:
        ...
        while not (terminated or truncated):
            if config["manual"]:
                ...
                # 获取当前速度方向（用于确定转向效果）
                current_speed = env.vehicle_state[3]
                steer_multiplier = 2.0  # 默认前进转向系数
                if current_speed < 0:  # 倒车时转向效果相反
                    steer_multiplier = -1.0
                
                # 转向控制 - 修正转向方向问题
                if keys[pygame.K_LEFT]:
                    action[0] = max(-1.0, action[0] - steer_step * steer_multiplier)
                elif keys[pygame.K_RIGHT]:
                    action[0] = min(1.0, action[0] + steer_step * steer_multiplier)
                else:
                    # 无转向按键时缓慢回正
                    if action[0] > 0:
