    ...
    def _simulate_radar(self):
        ...
        for geom in self.obstacle_geoms:
            if isinstance(geom, Point):
                ...
                points = [Point(c) for c in geom.coords]
            elif isinstance(geom, Polygon):
                points = [Point(c) for c in geom.exterior.coords]
            elif hasattr(geom, 'geoms'):  # MultiPolygon / MultiLineString
                points = []
                for g in geom.geoms:
                    if isinstance(g, LineString):
                        points.extend(Point(c) for c in g.coords)
                    elif isinstance(g, Polygon):
                        points.extend(Point(c) for c in g.exterior.coords)
            else:
                continue
            ...
            for pt in points:
                ...
                theta = math.atan2(dy, dx) - ego_yaw
                theta = (math.degrees(theta) + 360) % 360  # 转为 [0, 360)
                bin_idx = int(theta // self.angular_resolution)
                radar_data[bin_idx] = min(radar_data[bin_idx], r)
